...
export class GitHubFileTreeService {
     ...
     */
    async getRepositoryFileTree(owner, repo, branch = 'main') {
        ...
        try {
            ...
            const filteredItems = tree.tree.filter(item => item.path &&
                item.type &&
     ...
     */
    async downloadRepositoryToPath(owner, repo, localPath, branch = 'main') {
     ...
     */
    async downloadFileTreeItems(owner, repo, branch, items, basePath) {
        const fs = await import('fs');
        const path = await import('path');
        const fsPromises = fs.promises;
        ...
        for (const item of items) {
            ...
            if (pathParts.length === 1) {
                ...
                for (let i = 0; i < pathParts.length - 1; i++) {
                    ...
                    let existingDir = currentLevel.find(item => item.name === dirName && item.type === 'directory');
                    if (!existingDir) {
...
// Factory function
function createGitHubFileTreeService(accessToken) {
