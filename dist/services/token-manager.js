...
export class TokenManager {
    constructor(supabaseService, systemToken) {
        this.systemTokenLimit = 3; // Allow 3 free deployments
     ...
     */
    async selectToken(userId, userProvidedToken) {
     ...
     */
    async trackDeployment(userId, projectId, usedSystemToken) {
        if (!usedSystemToken)
            return;
        try {
     ...
     */
    async shouldPromptUpgrade(userId) {
        const tokenInfo = await this.getUserTokenInfo(userId);
     ...
     */
    async saveUserToken(userId, vercelToken) {
        try {
