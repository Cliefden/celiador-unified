...
class DatabaseService {
    ...
    async updateJobStatus(id, status, output, error, metadata) {
        if (!this.supabaseService)
            return null;
    ...
    async getConversationsByProjectId(projectId, userId) {
        if (!this.supabaseService)
            return [];
    ...
    async getPlatformGitHubIntegration(userId) {
    ...
    async removeGitHubIntegration(userId) {
        if (!this.supabaseService)
            return null;
    ...
    async initializeGitHubTrial(userId, trialDays = 30) {
        if (!this.supabaseService)
            throw new Error('Database not available');
        const now = new Date();
    ...
    async extendGitHubTrial(userId, additionalDays) {
        if (!this.supabaseService)
            throw new Error('Database not available');
    ...
    // Vercel integrations
    async getVercelIntegration(userId) {
        if (!this.supabaseService)
            return null;
        try {
            const { data, error } = await this.supabaseService
    ...
    async getPlatformVercelIntegration(userId) {
    ...
    async getRecentDeployments(projectId, limit = 5) {
        if (!this.supabaseService)
            return [];
        try {
            const { data, error } = await this.supabaseService
    ...
    async getDeploymentCount(userId, since) {
        if (!this.supabaseService)
            return 0;
        try {
    ...
    async initializeVercelTrial(userId, trialDays = 30) {
        if (!this.supabaseService)
            throw new Error('Database not available');
        const now = new Date();
        const trialExpires = new Date(now.getTime() + trialDays * 24 * 60 * 60 * 1000);
    ...
    async extendVercelTrial(userId, additionalDays) {
        if (!this.supabaseService)
            throw new Error('Database not available');
        // Get current trial info
        const { data: profile, error: profileError } = await this.supabaseService
    ...
    async getServiceIntegrations(userId) {
        if (!this.supabaseService)
            return [];
        const { data, error } = await this.supabaseService
    ...
    async updateDeployment(deploymentId, updates, userId) {
        if (!this.supabaseService)
            throw new Error('Database not available');
        const { data, error } = await this.supabaseService
            .from('deployments')
