export class BackupService {
     ...
     */
    async createBackup(userId, projectId, filePath, originalContent, operationType, jobId) {
        try {
     ...
     */
    async getBackupHistory(projectId, userId, limit = 50) {
        try {
            const { data, error } = await this.supabase
                .from('file_backups')
                .select('*')
     ...
     */
    async cleanupOldBackups(projectId, userId, keepCount = 10) {
        try {
     ...
     */
    async getFileContentForBackup(userId, projectId, filePath) {
        try {
