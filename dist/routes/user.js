    ...
    try {
        ...
        const [projects, conversations, jobs, integrations] = await Promise.all([
            ...
            supabaseService
                ...
                .in('projectid', await db.getProjectsByUserId(userId).then((projects) => projects.map((p) => p.id)))
                .then((result) => result.data || []),
            // User's integrations
        ...
        try {
            ...
            for (const project of projects) {
                try {
                    ...
                    const fileTree = await githubFileTreeService.getRepositoryFileTree(project.repoowner, project.reponame, 'main');
        ...
        const statistics = {
            ...
            totalDeployments: jobs.filter((job) => job.type === 'DEPLOY' && job.status === 'COMPLETED').length,
            storageUsed: storageUsed,
            apiCallsThisMonth: jobs.filter((job) => {
        ...
        // Get recent activity
        const recentJobs = jobs.filter((job) => new Date(job.createdat) >= weekAgo);
        const recentConversations = conversations.filter((conv) => new Date(conv.createdat) >= weekAgo);
        // Calculate streak (simplified - days with activity in last 30 days)
        const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
        const allActivity = [...jobs, ...conversations].filter((item) => new Date(item.createdat) >= thirtyDaysAgo);
        ...
        const activity = {
            lastActiveAt: allActivity.length > 0 ?
                Math.max(...allActivity.map((item) => new Date(item.createdat).getTime())) :
...
router.get('/api/user/activity', authenticateUser, async (req, res) => {
    try {
...
// Helper functions
function getJobTitle(type, status) {
    const typeNames = {
...
function getJobDescription(type, status, metadata) {
