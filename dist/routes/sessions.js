...
router.post('/api/sessions/start', authenticateUser, async (req, res) => {
    try {
        ...
        const { sessionId, platform, deviceInfo = {}, locationInfo = {}, userAgent } = req.body;
...
router.post('/api/sessions/heartbeat', authenticateUser, async (req, res) => {
    try {
        const { supabaseService } = getServices(req);
        if (!supabaseService) {
            return res.status(500).json({ error: 'Database not available' });
...
router.post('/api/sessions/end', authenticateUser, async (req, res) => {
    try {
...
router.get('/api/sessions/active', authenticateUser, async (req, res) => {
    try {
        const { supabaseService } = getServices(req);
        if (!supabaseService) {
            return res.status(500).json({ error: 'Database not available' });
...
router.get('/api/sessions/history', authenticateUser, async (req, res) => {
    try {
        const { supabaseService } = getServices(req);
        if (!supabaseService) {
            return res.status(500).json({ error: 'Database not available' });
...
router.get('/api/sessions/analytics', authenticateUser, async (req, res) => {
    try {
        const { supabaseService } = getServices(req);
        if (!supabaseService) {
            return res.status(500).json({ error: 'Database not available' });
        ...
        const totalDuration = endedSessions.reduce((sum, s) => sum + (s.total_duration_seconds || 0), 0);
...
router.post('/api/sessions/cleanup', authenticateUser, async (req, res) => {
    try {
        const { supabaseService } = getServices(req);
        if (!supabaseService) {
            return res.status(500).json({ error: 'Database not available' });
