...
router.get('/api/integrations/projects/:id/vercel-status', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const customerVercel = null; // TODO: Query customer Vercel integration
        const platformVercel = null; // TODO: Query platform Vercel integration
...
router.get('/api/integrations/projects/:id/github-status', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
        const { supabaseService, db } = getServices(req);
        const project = await db.getProjectById(id);
...
router.get('/api/integrations/projects/:id/status', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
        const { supabaseService, db } = getServices(req);
        const project = await db.getProjectById(id);
        ...
        const vercelIntegration = serviceIntegrations.find((s) => s.service === 'vercel');
        const response = {
            github: {
...
router.get('/api/integrations/projects/:id/deployment-options', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/api/integrations/projects/:id/deploy', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/api/integrations/vercel/teams', authenticateUser, async (req, res) => {
    try {
        // Mock Vercel teams
        const teams = [
            {
...
router.get('/api/integrations/vercel/projects', authenticateUser, async (req, res) => {
    try {
...
router.post('/api/integrations/projects/:id/vercel', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
...
router.delete('/api/integrations/projects/:id/vercel', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
        const { db } = getServices(req);
        const project = await db.getProjectById(id);
...
router.post('/api/integrations/projects/:id/vercel-platform-connect', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
        const { db } = getServices(req);
        const project = await db.getProjectById(id);
...
router.post('/api/integrations/projects/:id/vercel-disconnect', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/api/integrations/projects/:id/vercel-deploy', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
        const { type } = req.body; // 'customer' or 'platform'
                ...
                if (repoResponse.status === 404) {
                    ...
                    if (!project.repo_created) {
                        try {
                            ...
                            const conflictingProject = existingProjects.find((p) => p.id !== project.id &&
                            ...
                            try {
                                ...
                                const { data: allFiles, error: listError } = await req.app.locals.supabaseService.storage
                                    .from('project-files')
                                    .list(project.id, { limit: 1000 });
                                if (listError || !allFiles || allFiles.length === 0) {
                                    console.warn(`[VERCEL] No files found in Supabase Storage for project ${project.id}`);
                                }
                                else {
                                    console.log(`[VERCEL] Found ${allFiles.length} files to push to GitHub`);
                                    // Download all files and prepare for GitHub push
                                    const filesToPush = [];
                                    for (const file of allFiles) {
                                        try {
                                            const { data: fileData, error: downloadError } = await req.app.locals.supabaseService.storage
                                                .from('project-files')
                                                .download(`${project.id}/${file.name}`);
                                            ...
                                            console.warn(`[VERCEL] Failed to download file ${file.name}:`, fileError);
                                        }
                                    }
                                    if (filesToPush.length > 0) {
                                        // Push all files to GitHub at once
                                        await githubService.createBatchCommit(project.repoowner, project.reponame, filesToPush, 'Initial commit: Deploy project files');
        ...
        try {
            ...
            const deployment = await vercelService.deployFromFiles(projectName, project.repoowner, project.reponame, defaultBranch, 'production');
...
router.get('/api/integrations/projects/:id/vercel-deployment/:deploymentId', authenticateUser, async (req, res) => {
    try {
...
router.post('/api/integrations/projects/:id/github-disconnect', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
        const { type } = req.body; // 'customer' or 'platform'
        const { supabaseService, db } = getServices(req);
