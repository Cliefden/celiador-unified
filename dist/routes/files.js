...
async function getTemplateFileStructure(templateKey) {
...
async function getTemplateFileContent(path, project) {
    const templates = {
...
router.get('/projects/:id/files/tree', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        console.log(`[FileTree] Building file tree for project ${project.name} from Supabase Storage`);
        try {
            // Recursively get all files from Supabase Storage
            const getAllFiles = async (prefix = '') => {
                ...
                if (error || !items) {
                    console.warn(`[FileTree] Failed to list path ${path}:`, error?.message);
                    return [];
                }
                const allFiles = [];
                for (const item of items) {
                    ...
                    if (item.metadata === null && item.id === null) {
                        ...
                        console.log(`[FileTree] Found directory: ${itemPath}, recursing...`);
                        const subFiles = await getAllFiles(itemPath);
                        allFiles.push(...subFiles);
                    }
                    else {
                        // This is a file
                        console.log(`[FileTree] Found file: ${itemPath}`);
                        allFiles.push({
                            ...item,
            ...
            if (!storageFiles || storageFiles.length === 0) {
                console.warn(`[FileTree] No files in storage for project ${id} - project may not be fully initialized`);
                return res.json({ tree: [] });
            }
            console.log(`[FileTree] Found ${storageFiles.length} total files in storage for project ${id}`);
            console.log(`[FileTree] Sample files:`, storageFiles.slice(0, 5).map(f => ({ name: f.name, size: f.metadata?.size })));
            ...
            const fileTree = buildFileTree(storageFiles);
            console.log(`[FileTree] ✅ Built file tree from storage with ${fileTree.length} root items`);
...
router.get('/projects/:id/files/:path(*)', authenticateUser, async (req, res) => {
    try {
        ...
        try {
            // SINGLE SOURCE OF TRUTH: Supabase Storage only
            console.log(`[FileContent] Getting file ${path} from Supabase Storage for project ${id}`);
            // Encode path to handle special characters like brackets
            const encodedPath = path.replace(/\[/g, '%5B').replace(/\]/g, '%5D');
            const { data, error } = await supabaseService.storage
                .from('project-files')
                .download(`${id}/${encodedPath}`);
            if (error || !data) {
                console.warn(`[FileContent] File not found in storage: ${error?.message || 'No data'}`);
                return res.status(404).json({ error: 'File not found' });
            }
            const content = await data.text();
            console.log(`[FileContent] ✅ Got file from Supabase Storage (${content.length} chars)`);
            return res.json({ content, path, updatedAt: new Date().toISOString() });
        }
        catch (storageError) {
            console.error(`[FileContent] Storage access failed for ${path}:`, storageError);
...
router.post('/projects/:id/files/save', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/projects/:id/files/create', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
...
router.delete('/projects/:id/files/:path(*)', authenticateUser, async (req, res) => {
    try {
        const { id, path } = req.params;
        const { db } = getServices(req);
        const project = await db.getProjectById(id);
...
router.post('/projects/:id/files/delete-folder', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
        const { path } = req.body;
        const { db } = getServices(req);
...
router.post('/projects/:id/files/upload', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
