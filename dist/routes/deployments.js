...
router.post('/api/deployments/deploy', authenticateUser, async (req, res) => {
    try {
        ...
        const { projectId, projectName, templateKey, files, isPrivate = false, orgName, githubToken, vercelToken // Optional - if not provided, will use TokenManager logic
         } = req.body;
...
router.get('/api/deployments/status/:projectId', authenticateUser, async (req, res) => {
    try {
        const { supabaseService } = getServices(req);
        if (!supabaseService) {
            return res.status(500).json({ error: 'Database not available' });
...
router.post('/api/deployments/vercel/projects', authenticateUser, async (req, res) => {
    try {
...
router.get('/api/deployments/vercel/projects', authenticateUser, async (req, res) => {
    try {
...
router.post('/api/deployments/vercel/deploy', authenticateUser, async (req, res) => {
    try {
...
router.get('/api/deployments/vercel/deployments/:deploymentId', authenticateUser, async (req, res) => {
    try {
...
router.get('/api/deployments/vercel/deployments/:deploymentId/logs', authenticateUser, async (req, res) => {
    try {
        const { deploymentId } = req.params;
        const { vercelToken } = req.query;
        if (!vercelToken) {
...
router.delete('/api/deployments/vercel/deployments/:deploymentId', authenticateUser, async (req, res) => {
    try {
        const { deploymentId } = req.params;
...
router.post('/api/deployments/vercel/validate-token', authenticateUser, async (req, res) => {
    try {
        const { vercelToken } = req.body;
        if (!vercelToken) {
            return res.status(400).json({ error: 'Vercel API token is required' });
...
router.get('/api/deployments/vercel/projects/:projectId/deployments', authenticateUser, async (req, res) => {
    try {
        const { projectId } = req.params;
        ...
        const vercelService = new VercelService(vercelToken);
        const deployments = await vercelService.getProjectDeployments(projectId, parseInt(limit));
        res.json({
            success: true,
            deployments
        });
...
router.get('/api/deployments/token-info', authenticateUser, async (req, res) => {
    try {
        const { supabaseService } = getServices(req);
        if (!supabaseService) {
            return res.status(500).json({ error: 'Database not available' });
...
router.post('/api/deployments/save-token', authenticateUser, async (req, res) => {
    try {
        const { supabaseService } = getServices(req);
        if (!supabaseService) {
            return res.status(500).json({ error: 'Database not available' });
...
router.delete('/api/deployments/remove-token', authenticateUser, async (req, res) => {
    try {
        const { supabaseService } = getServices(req);
        if (!supabaseService) {
            return res.status(500).json({ error: 'Database not available' });
...
router.post('/api/deployments/github-to-vercel', authenticateUser, async (req, res) => {
    try {
        const { projectName, githubToken, vercelToken, repoOwner, repoName, branch = 'main', target = 'production' } = req.body;
        ...
        const vercelService = new VercelService(vercelToken);
        const deployment = await vercelService.deployFromGitHub(projectName, githubToken, repoOwner, repoName, branch, target);
        res.status(201).json({
            success: true,
            deployment
        });
