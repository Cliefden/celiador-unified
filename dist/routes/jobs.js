...
import { formatEntityAsActivity } from '../utils/activityFormatter.js';
...
router.get('/projects/:id/jobs', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/projects/:id/jobs', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/jobs/:id/cancel', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
        const { db, jobService } = getServices(req);
...
router.post('/jobs/:id/retry', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/projects/:id/activity', authenticateUser, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        try {
            const { data: conversations } = await supabaseService
                ...
                .order('createdat', { ascending: false })
                .limit(5);
            conversationActivities = conversations?.map((conv) => formatEntityAsActivity('conversations', conv, 'created')) || [];
        }
        catch (convError) {
        ...
        const jobActivities = jobs?.map((job) => formatEntityAsActivity('jobs', job, 'updated', req.user.id)) || [];
