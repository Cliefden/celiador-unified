...
export class AIService {
     ...
     */
    async generateResponse(messages, config = { provider: 'openai' }) {
     ...
     */
    async generateOpenAIResponse(messages, model = 'gpt-4o-mini') {
        if (!this.openai) {
     ...
     */
    async generateAnthropicResponse(messages, model = 'claude-3-haiku-20240307') {
        if (!this.anthropic) {
     ...
     */
    async getRelevantDocumentation(userMessage) {
     ...
     */
    async createDevelopmentSystemPrompt(userMessage, projectContext) {
