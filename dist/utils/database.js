...
export function initializeSupabase() {
    try {
        if (process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY) {
            supabase = createClient(process.env.SUPABASE_URL, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || process.env.SUPABASE_ANON_KEY || '');
...
export const db = {
    ...
    async getConversationsByProject(projectId, userId) {
        if (!supabaseService)
            return [];
        const { data, error } = await supabaseService
            .from('conversations')
