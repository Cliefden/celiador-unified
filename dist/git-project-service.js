...
export class GitProjectService {
     ...
     */
    async initializeProject(projectId, projectName, templateRepoUrl, userRepoUrl, userId) {
     ...
     */
    async copyTemplateToProject(templatePath, projectPath) {
        const skipPatterns = [
            '.git',
            'node_modules',
            '.next',
     ...
     */
    async initializeGitRepo(projectPath, projectName) {
        const commands = [
            'git init',
            'git add .',
...
🚀 Generated with Celiador"`
     ...
     */
    async setupRemoteAndPush(projectPath, repoUrl) {
        const commands = [
     ...
     */
    async cloneProject(projectId, repoUrl, branch = 'main') {
        const projectPath = path.join(this.tempDir, projectId);
     ...
     */
    async writeProjectFile(projectPath, filePath, content) {
        try {
            const fullPath = path.join(projectPath, filePath);
     ...
     */
    async getProjectFileTree(projectPath) {
        const skipPatterns = [
            '.git',
            'node_modules',
            '.next',
     ...
     */
    async commitAndPush(projectPath, message) {
        const commands = [
            'git add .',
     ...
     */
    static generateRepoName(projectName) {
        return projectName
            .toLowerCase()
            .replace(/[^a-z0-9\s-]/g, '')
            .replace(/\s+/g, '-')
