      ...
      if (!repoResponse.ok) {
        if (repoResponse.status === 404) {
          ...
          return res.status(400).json({ 
            ...
            message: `The GitHub repository ${project.repoowner}/${project.reponame} does not exist. Please create the repository first or update the project configuration.`
          });
        } else if (repoResponse.status === 403) {
